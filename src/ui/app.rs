...
use crate::{ColorPalette, PaletteOptions};
...
pub struct App {
    ...
    pub palette_options: PaletteOptions,
...
impl App {
    pub fn new() -> Self {
        Self::with_options(PaletteOptions::default(), ExportOptions::default())
    }

    pub fn with_options(palette_options: PaletteOptions, export_options: ExportOptions) -> Self {
        Self {
    ...
    fn generate_palette(&mut self) {
        ...
        match ColorPalette::new_with_options(input, &self.palette_options) {
            Ok(palette) => {
                self.current_palette = Some(palette);
                self.state = AppState::PaletteView;
            }
            Err(_) => {
                self.message = Some("Invalid color format. Use #RGB or #RRGGBB".to_string());
            }
        }
    }
    ...
    fn export_css(&mut self) {
        if let Some(palette) = &self.current_palette {
            if let Ok(content) =
                crate::export::ExportFormat::Css.export_with_options(palette, &self.export_options)
            {
    ...
    fn export_tailwind_v3(&mut self) {
        if let Some(palette) = &self.current_palette {
            if let Ok(content) = crate::export::ExportFormat::TailwindV3
    ...
    fn export_tailwind_v4(&mut self) {
        if let Some(palette) = &self.current_palette {
            if let Ok(content) = crate::export::ExportFormat::TailwindV4
                .export_with_options(palette, &self.export_options)
            {
    ...
    fn export_json(&mut self) {
        if let Some(palette) = &self.current_palette {
            if let Ok(content) =
                crate::export::ExportFormat::Json.export_with_options(palette, &self.export_options)
            {
