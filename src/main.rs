...
struct Args {
    #[arg(short, long, help = "Initial color in hex format")]
    color: Option<String>,
}
...
fn render_input_screen(f: &mut Frame, app: &App) {
    ...
    // Live preview
    if let Ok(color) = parse_hex(&app.color_input) {
...
fn parse_hex(hex: &str) -> Result<palette::Srgb<f32>, ()> {
